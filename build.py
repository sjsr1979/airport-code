...
NAV_LOGO_SVG = '''<svg width="32" height="32" viewBox="0 0 34 34" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Airport Code logo"><rect width="34" height="34" rx="9" fill="#1a56db"/><path d="M24.5 8.5C25.3 9.3 25.3 10.6 24.5 11.4L19.8 16.1L22 25L19.5 27.5L16 19L11.5 23.5L12 26L10 28L8 24L4 22L6 20L8.5 20.5L13 16L5.5 12.5L8 10L17 12.2L21.6 7.5C22.4 6.7 23.7 6.7 24.5 8.5Z" fill="white"/></svg>'''
...
SHARED_CSS = '''
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .mobile-menu { display: none; position: fixed; top: 60px; left: 0; right: 0; bottom: 0; background: var(--navy); z-index: 99; flex-direction: column; padding: 24px; gap: 8px; }
    .mobile-menu.open { display: flex; }
    .mobile-menu a { color: #fff; text-decoration: none; font-size: 18px; font-weight: 600; padding: 14px 0; border-bottom: 1px solid rgba(255,255,255,0.1); }
    .mobile-menu .mob-search { display: flex; align-items: center; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); border-radius: 10px; padding: 10px 14px; gap: 10px; margin-bottom: 8px; }
    .mobile-menu .mob-search input { background: none; border: none; outline: none; color: #fff; font-size: 15px; width: 100%; font-family: \'Outfit\', sans-serif; }
    .mobile-menu .mob-search input::placeholder { color: rgba(255,255,255,0.4); }
    #mob-search-results { background: rgba(255,255,255,0.05); border-radius: 10px; overflow: hidden; }
    #mob-search-results a { display: flex; align-items: center; gap: 10px; padding: 10px 14px; text-decoration: none; color: #fff; font-size: 13px; border-bottom: 1px solid rgba(255,255,255,0.08); }
    #mob-search-results a:last-child { border-bottom: none; }
    #mob-search-results .sr-code { background: var(--blue); color: #fff; font-weight: 700; font-size: 12px; padding: 2px 7px; border-radius: 5px; min-width: 38px; text-align: center; }
    #mob-search-results .sr-name { font-weight: 600; }
    #mob-search-results .sr-city { color: rgba(255,255,255,0.5); font-size: 11px; }
    ...
    .carbon-form input, .carbon-form select { width: 100%; padding: 10px 12px; border: 2px solid #94a3b8; border-radius: 8px; font-size: 14px; font-family: \'Outfit\', sans-serif; color: var(--text); outline: none; transition: border-color 0.2s; }
...
<div class="mobile-menu" id="mobileMenu">
  <div class="mob-search">
    ...
    <input type="text" id="mobSearchInput" placeholder="Search airport or code…" autocomplete="off">
  </div>
  <div id="mob-search-results"></div>
  <a href="/az" onclick="closeMenu()">A–Z Index</a>
  <a href="/countries" onclick="closeMenu()">By Country</a>
...
(function(){
  ...
  const results = document.getElementById('mob-search-results');
  if (!input) return;
  async function loadDB() {
    if (db) return;
    const root = document.querySelector('meta[name="site-root"]')?.content || '/';
  ...
  input.addEventListener('input', async function() {
    ...
    const root = document.querySelector('meta[name="site-root"]')?.content || '/';
    results.innerHTML = matches.map(a =>
      `<a href="${root}${a.iata.toLowerCase()}/index.html" onclick="closeMenu()">
        <span class="sr-code">${a.iata}</span>
        <div><div class="sr-name">${a.name}</div><div class="sr-city">${a.city}, ${a.country}</div></div>
      </a>`
    ).join('');
...
function setTrip(v){{
  document.getElementById('tripVal').value=v;
  document.getElementById('trip-oneway').style.background=v===1?'#2563eb':'#fff';
  document.getElementById('trip-oneway').style.color=v===1?'#fff':'#475569';
  document.getElementById('trip-return').style.background=v===2?'#2563eb':'#fff';
  document.getElementById('trip-return').style.color=v===2?'#fff':'#475569';
}}
...
async function calcCarbon(){{
  ...
  const tripMult = parseFloat(document.getElementById('tripVal').value);
...
<script>
(function(){{
  const url=`https://airport-code.com/api/adsb/lat/{lat:.4f}/lon/{lon:.4f}/dist/50`;
  const container=document.getElementById('flights-container');
  fetch(url).then(r=>r.json()).then(d=>{{
    const ac=(d.ac||[]).filter(a=>a.flight&&a.flight.trim()&&!a.ground);
    if(!ac.length){{
      container.innerHTML='<p style="color:#94a3b8;font-size:13px;text-align:center;padding:12px">No aircraft currently overhead.</p>';
      return;
    }}
    ac.sort((a,b)=>(b.gs||0)-(a.gs||0));
    const shown=ac.slice(0,8);
    const header=`<div style="display:flex;justify-content:space-between;align-items:center;padding:6px 16px;border-bottom:2px solid #e2e8f0;background:#f8fafc">
      <div style="font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.05em">Flight</div>
      <div style="font-size:11px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.05em">Altitude</div>
    </div>`;
    const rows=shown.map(a=>{{
      const callsign=(a.flight||'').trim();
      const reg=a.r||'';
      const type=a.t||'';
      const altVal=typeof a.alt_baro==='number'?a.alt_baro:null;
      const alt=altVal!=null?Math.round(altVal).toLocaleString()+' ft':'—';
      ...
      return `<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 16px;border-bottom:1px solid #f0f0f0;gap:8px">
        ...
        <div style="font-size:13px;font-weight:600;color:#1e293b;white-space:nowrap;flex-shrink:0">${{alt}}</div>
      </div>`;
    }}).join('');
    document.getElementById('flights-container').innerHTML=header+rows;
  }}).catch(()=>{{
...
def airport_page(a):
    ...
    for fr in freqs:
        ...
        <tr style="border-bottom:1px solid #f5f5f5">
          ...
          <td class="freq-desc" style="padding:10px 16px;font-size:13px;color:var(--muted)">{escape_html(fr["description"])}</td>
      ...
      <div class="card-body" style="padding:0">
        <table style="width:100%;border-collapse:collapse">
          <thead><tr style="background:#f8fafc">
            ...
            <th class="freq-desc" style="padding:10px 16px;text-align:left;font-size:11px;color:#888;text-transform:uppercase;letter-spacing:0.5px;font-weight:600;border-bottom:1px solid #e2e8f0">Description</th>
...
<div class="main">
  <div class="content">
    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="carbon-form">
          ...
          <div style="display:flex;gap:0;border:1px solid #cbd5e1;border-radius:8px;overflow:hidden;width:fit-content">
            <button type="button" id="trip-oneway" onclick="setTrip(1)" style="padding:8px 20px;font-size:14px;font-weight:600;border:none;cursor:pointer;background:#2563eb;color:#fff">One way</button>
            <button type="button" id="trip-return" onclick="setTrip(2)" style="padding:8px 20px;font-size:14px;font-weight:600;border:none;cursor:pointer;background:#fff;color:#475569">Return</button>
          </div>
          <input type="hidden" name="trip" id="tripVal" value="1">
    ...
    <div class="card">
      ...
      <div style="padding:10px 16px;font-size:11px;color:#94a3b8;border-top:1px solid var(--border);">Aircraft currently within 50nm of {iata}. Data via <a href="https://adsb.lol" target="_blank" rel="noopener" style="color:#94a3b8">adsb.lol</a>.</div>
    </div>
...
<div class="stats-bar">
  ...
  <div class="stat-item"><div class="num">Live</div><div class="lbl">Airport Data</div></div>
...
<head>
  ...
  <link rel="canonical" href="https://airport-code.com/az/">
...
<head>
  ...
  <link rel="canonical" href="https://airport-code.com/countries/">
