...
SHARED_CSS = '''
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    body { font-family: \'Outfit\', sans-serif; background: var(--bg); color: var(--text); font-size: 15px; line-height: 1.6; zoom: 1.12; }
    ...
    @media (max-width: 768px) { .main { grid-template-columns: 1fr; } .hero-name h1 { font-size: 26px; } .nav-links { display: none; } .nav-search { width: 200px; } }
    ...
    .card-header h2 { font-size: 14px; font-weight: 700; color: var(--text); text-transform: uppercase; letter-spacing: 0.5px; }
...
async function calcCarbon(){{
  const dest = document.getElementById('destInput').value.toUpperCase().trim();
  ...
  if (!destAirport) {{
    co2El.textContent = '?';
    noteEl.textContent = `Airport code "${{dest}}" not found. Try a 3-letter IATA code like JFK, DXB, SYD.`;
  ...
  noteEl.textContent = `${{ORIGIN.iata}} → ${{dest}} is approx ${{Math.round(distKm).toLocaleString()}} km. ${{classNames[String(classMult)]}} ${{tripName}} flight.`;
  resultEl.style.display = 'block';
}}
</script>'''
...
<div class="main">
  <div class="content">
      ...
      <div class="card-body">
        ...
        <div class="carbon-form">
          <div>
            ...
            <input type="text" id="destInput" placeholder="e.g. JFK, DXB, SYD" maxlength="4" style="text-transform:uppercase">
          </div>
          <div>
    ...
    <div class="card">
      <div class="card-header"><span>✈️</span><h2>Live Aircraft Overhead <span style="font-size:11px;font-weight:500;color:#10b981;background:#f0fdf4;border:1px solid #bbf7d0;padding:2px 8px;border-radius:20px;margin-left:8px;">● LIVE</span></h2></div>
      <div class="card-body" style="padding:0">
        <div id="flights-container" style="padding:20px;color:#94a3b8;font-size:13px;text-align:center;">Loading live traffic…</div>
        <div style="padding:10px 16px;border-top:1px solid #e2e8f0;text-align:center;">
          <p style="font-size:11px;color:#94a3b8;">Live aircraft via OpenSky Network · Within ~130km · <a href="https://www.flightaware.com/live/airport/{icao}" target="_blank" rel="noopener" style="color:#1a56db;font-weight:600;">Full departures on FlightAware ↗</a></p>
        </div>
      </div>
    </div>
